...
Page({
  ...
  async drawData(levelsData, ctx, canvas, type) {
    ...
    levelsData.forEach((item, index) => {
      if (item.selected) haveImgIdx = index
    })
    for (let i = 0; i < levelsData.length; i++) {
      ...
      if (type === 'image') {
        ...
        if (item.selected && i === haveImgIdx) {
          selectedImageIndex = canvasData.images.findIndex(img => img.id === item.id)
  ...
  selectIcon(touchX, touchY, selectType) {
    ...
    const delTouchVal = data.rotate
      ? utils.rotatePoint(data.x, data.y, centerX, centerY, data.rotate)
      : {
          x: data.x,
          y: data.y
        }
    const dragTouchVal = data.rotate
      ? utils.rotatePoint(data.x + data.width, data.y + data.height, centerX, centerY, data.rotate)
      : {
          x: data.x + data.width,
    ...
    if (touchX >= delTouchX - 15 && touchX <= delTouchX + 15 && touchY >= delTouchY - 15 && touchY <= delTouchY + 15) {
